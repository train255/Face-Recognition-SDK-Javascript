...
export async function load_opencv() {
...
export async function load_pose_model() {
  var pose_session = null
  await ort.InferenceSession.create("../model/fr_pose.onnx", {executionProviders: ['wasm']})
      .then((session) => {
        pose_session = session
        const input_tensor = new ort.Tensor("float32", new Float32Array(224 * 224 * 3), [1, 3, 224, 224]);
...
export async function load_detection_model() {
  var detect_session = null;
  await ort.InferenceSession.create("../model/fr_detect.onnx", {executionProviders: ['wasm']})
      .then((session) => {
        detect_session = session;
        const input_tensor = new ort.Tensor("float32", new Float32Array(320 * 240 * 3), [1, 3, 240, 320]);
...
export async function load_live_model() {
  var live_session = null
  await ort.InferenceSession.create("../model/fr_liveness.onnx", {executionProviders: ['wasm']})
      .then((session) => {
        live_session = session
        const input_tensor = new ort.Tensor("float32", new Float32Array(128 * 128 * 3), [1, 3, 128, 128]);
...
export async function load_landmark_model() {
  var landmark_session = null
  await ort.InferenceSession.create("../model/fr_landmark.onnx", {executionProviders: ['wasm']})
      .then((session) => {
        landmark_session = session
        const input_tensor = new ort.Tensor("float32", new Float32Array(64 * 64), [1, 1, 64, 64]);
...
export async function load_eye_model() {
  var eye_session = null
  await ort.InferenceSession.create("../model/fr_eye.onnx", {executionProviders: ['wasm']})
      .then((session) => {
        eye_session = session
        const input_tensor = new ort.Tensor("float32", new Float32Array(24 * 24 * 1), [1, 1, 24, 24]);
...
export async function load_expression_model() {
  var expression_session = null
  await ort.InferenceSession.create("../model/fr_expression.onnx", {executionProviders: ['wasm']})
      .then((session) => {
...
export function preprocess_detection(image) {
  var rows = image.rows,
      cols = image.cols;
...
export async function detect_photo(session, canvas_id) {
  const onnx_config = {
  ...
  const image = preprocess_detection(img);
  ...
  const priors = define_prior_box([320, 240], onnx_config);

  const boxes_arr = decode_bbox(loc, priors, onnx_config);
  ...
  var boxes_before = scale_multiply_bbox(box, scale);
  var landms_before = null;//scale_multiply_landms(landms_arr, scale1);

  var [bbox_screen, scores_screen, landms_screen] = screen_score(boxes_before, scores_arr, landms_before, onnx_config.confidence_threshold);

  var [bbox_sorted, scores_sorted, landms_sorted] = sort_score(bbox_screen, scores_screen, landms_screen, onnx_config.top_k);

  var [bbox_small, score_result, landms_small, result_size] = cpu_nms(bbox_sorted, scores_sorted, landms_sorted, onnx_config.nms_threshold);
...
export function define_prior_box(image_size, onnx_config) {
...
export function decode_bbox(bbox, priors, onnx_config) {
...
export function decode_landmark(landmark, priors, variances) {
...
export function scale_multiply_bbox(boxes_arr, scale) {
...
export function scale_multiply_landms(landms_arr, scale1) {
...
export function screen_score(bbox, scores, landms, threshold) {
...
export function sort_score(bbox, scores, landms, top_k) {
  var total_size = scores.shape[0];
...
export function cpu_nms(bbox, scores, landms, thresh) {
  var {max, min} = Math;
...
export function result_scaling(bbox, landmark, resize_param) {
  var size = bbox.shape[0];
...
export function preprocess_pose(img) {
  var cols = img.cols;
  var rows = img.rows;
  var channels = 3;
...
export async function predict_pose(session, canvas_id, bbox, question) {
  var face_count = bbox.shape[0],
      bbox_size = bbox.shape[1]

  var img = cv.imread(canvas_id)

  var best_face_idx = get_best_face(bbox);
  if (best_face_idx === -1)
    return null;
  ...
  var input_image = preprocess_pose(resize_image)
...
// liveness detection
export function align_liveness_image(image, bbox, scale_value) {
...
export function preprocess_liveness(img) {
  var cols = img.cols;
  var rows = img.rows;
  var channels = 3;
...
export function get_best_face(bbox) {
  var face_count = bbox.shape[0],
      bbox_size = bbox.shape[1];
...
export async function predict_liveness(session, canvas_id, bbox) {
  var img = cv.imread(canvas_id)
  ...
  var face_img = align_liveness_image(img, [x1, y1, width, height], 2.7);
  //cv.imshow("live-temp", face_img);
  var input_image = preprocess_liveness(face_img);
  face_img.delete();
...
export function align_landmark_image(image, bbox, scale_value) {
  var src_h = image.rows,
      src_w = image.cols;
...
export function preprocess_landmark(img) {
  var cols = img.cols;
  var rows = img.rows;
  var channels = 1;
...
export async function predict_landmark(session, canvas_id, bbox) {
  var img = cv.imread(canvas_id);
  ...
  var face_img = align_landmark_image(img, [x1, y1, width, height], 1.2);
  // cv.imshow("live-temp", face_img);
  var input_image = preprocess_landmark(face_img);
  face_img.delete();
  const input_tensor = new ort.Tensor("float32", new Float32Array(64 * 64), [1, 1, 64, 64]);
...
export function get_eye_bbox(landmark, size) {
...
export function align_eye_image(image, landmark) {
  var src_h = image.rows,
      src_w = image.cols;

  var eye_bbox = get_eye_bbox(landmark, [src_h, src_w])
...
export function preprocess_eye(imgs) {
...
export async function predict_eye(session, canvas_id, landmark) {
  var img = cv.imread(canvas_id)

  var face_imgs = align_eye_image(img, landmark);
  // cv.imshow("live-temp", face_imgs[1]);
  var input_images = preprocess_eye(face_imgs);
  face_imgs[0].delete();
  face_imgs[1].delete();

  const input_tensor1 = new ort.Tensor("float32", new Float32Array(24 * 24), [1, 1, 24, 24]);
  ...
  const input_tensor2 = new ort.Tensor("float32", new Float32Array(24 * 24), [1, 1, 24, 24]);
...
export function align_expression_image(image, bbox) {
  var src_h = image.rows,
      src_w = image.cols;
...
export function preprocess_expression(img) {
  var cols = img.cols;
  var rows = img.rows;
  var channels = 3;
...
export async function predict_expression(session, canvas_id, bbox) {
  var img = cv.imread(canvas_id)
  ...
  var face_img = align_expression_image(img, [x1, y1, width, height]);
  //cv.imshow("live-temp", face_img);
  var input_image = preprocess_expression(face_img);
  face_img.delete();
